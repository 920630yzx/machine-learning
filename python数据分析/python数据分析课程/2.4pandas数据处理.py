...
df[cond].dropna(axis=1,how='all')  # 删除一列中全部为nan的列
...
cond.any(axis=1)    # 行中若有true则返回true，反之返回false
index_1 = df[cond.any(axis=1)].index  # 这是需要删除的行的索引
index_2 = index.difference(index_1)   # 求差集 index - index_1；也就是结果
print(df.drop(index_2))  # 需要删除的行
list(index_2)
df.loc[list(index_2)]   # 得到结果
df.drop(index_1)        # 当然这也是结果
...
# 4.1 take与loc/iloc的差别---take可以理解为提取,可以调整输出的顺序
...
df.iloc[[2,3]]  # 取出第二、三行
df.take([2])     # 取出第三行
df.take([2,3,1]) # 取出三行,take也可以调整输出的顺序
...
type(indices)
df2.take(indices)  # 随机抽样的结果,随机抽取10行的数据
...
# 5.1 groupby分类---groupby函数之后（分组之后）一定要作聚合操作（min，max,sum）
