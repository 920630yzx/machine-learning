...
# 10.1 dataframe的级联--使用pd.concat()级联
df1 = make_df([1,2],list('AB'))
df2 = make_df([3,4],list('AB'))
print(df1)
print(df2)
pd.concat([df1,df2])  # 将两个datafram联合得到一个全新的dataframe,默认在列的方向进行级联
pd.concat([df1,df2],axis = 1)  # 在行的方向进行级联
pd.concat([df1,df2],ignore_index=True)  # 在列的方向进行级联，且忽视索引

'''numpy的级联：
np.concatenate((df1,df2),axis = 1)   注意与pandas要区分开'''

# 10.2 dataframe的级联--使用多层索引keys进行级联
pd.concat([df1,df2],keys=['x','y'])

# 10.3 不匹配级联：不匹配指的是级联的维度的索引不一致。
...
pd.concat((df1,df2),join = 'inner',axis = 1)  # 内连接：只连接匹配的项，即行列的索引都相同
...
# 10.5 dataframe的级联---使用pd.merge()级联
...
df1.merge(df2)  # 第一个dataframe中有两个'张三'对应于第二个'name'中有两个'张三'
# 结果会分别对应，返回2*2=4个结果

# 10.5.4 key的规范化: 使用on=显式指定哪一列为key,当有多个列名相同时使用
df1 = DataFrame({'name':['张三','李四','张三'],'salary':[10000,12000,20000],'age':[22,21,25]})
...
# 10.5.5 left_on='name',right_on='名字'强行连接,左边是name列右边是名字列
df1 = DataFrame({'name':['张三','李四','张三'],'salary':[10000,12000,20000],'age':[22,21,25]})
...
# 10.5.6  强行连接,左边是age列右边是right_index=True表示索引列
df1 = DataFrame({'name':['张三','李四','张三'],'salary':[10000,12000,20000],'age':[22,21,25]})
